...
const SkyAtmosphere = {
    ...
    state: {
        isDay: 1,
        condition: 'clear', // 'clear', 'partlyCloudy', 'cloudy', 'rain', 'thunderstorm', 'snow', 'fog'
        weatherCode: 0,
        windSpeed: 10,
     ...
     */
    setWeather(weatherCode, isDay = 1, windSpeed = 10) {
        ...
        const dayChanged = this.state.isDay !== numericIsDay;
    ...
    applyAtmosphereClasses() {
        ...
        document.body.classList.toggle('theme-night', !isDay || cond === 'thunderstorm' || cond === 'heavyRain');
    },

    /**
     * Initialize Starfield for night sky
     */
    initStars() {
        this.stars = [];
        this.shootingStars = [];
    ...
    renderCelestial() {
        ...
        this.celestialContainer.innerHTML = '';
        this.celestialContainer.className = 'sky-celestial ' + (isDay ? 'celestial-day' : 'celestial-night');
    ...
    updateAndDraw() {
        ...
        // 1. Draw Starfield & Meteors (Night only, omitted during stormy weather)
        if (this.state.isDay === 0 && cond !== 'thunderstorm' && cond !== 'heavyRain') {
            this.drawStars();
